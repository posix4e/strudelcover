...
class MLAudioAnalyzer:
    ...
    def analyze(self, audio_path, output_path=None):
        ...
        patterns = self._extract_patterns(waveform[0], sr, results.get("transcription", {}))
    ...
    def _separate_sources(self, audio_path):
        ...
        for stem, audio in separated.items():
            ...
            demucs.api.save_audio(audio, stem_path, samplerate=self.demucs_model.samplerate)
            
            # Analyze each stem
            stem_audio = audio.mean(0).numpy()  # Convert to mono numpy
            stem_features = self._analyze_stem(stem_audio, self.demucs_model.samplerate, stem)
    ...
    def _analyze_structure(self, audio, sr):
        ...
        # Create sections
        sections = []
        section_names = ["intro", "verse1", "chorus1", "verse2", "chorus2", 
    ...
    def _extract_patterns(self, audio, sr, transcription):
        ...
        if len(onset_times) > 1:
            ...
            patterns["rhythmic"] = {
                "common_intervals": np.round(np.bincount(
                    np.round(ioi * 10).astype(int)
                ).argsort()[-5:] / 10, 3).tolist(),
        ...
        if transcription and transcription.get("notes"):
            ...
            # Pitch intervals
            if len(notes) > 1:
                intervals = [notes[i+1]["pitch"] - notes[i]["pitch"] 
        ...
        if chord_frames:
            patterns["harmonic"] = {
                ...
                "chord_changes": len([i for i in range(1, len(chord_frames)) 
    ...
    def _print_summary(self, results):
        ...
        if "stems" in results:
            ...
            for stem, data in results["stems"].items():
                ...
                print(f"  {stem}: tempo={features.get('tempo', 'N/A')}, "
        ...
        if "transcription" in results:
            ...
            print(f"  Range: {trans['pitch_range']['min']}-{trans['pitch_range']['max']}")
        ...
        if "structure" in results:
            ...
            for section in struct['sections'][:5]:
                print(f"    {section['name']}: {section['start']:.1f}s - {section['end']:.1f}s")
